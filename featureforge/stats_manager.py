...
BOOKING_DURATION = 210 * 60  # Measured in seconds
DB_NAME = 'experiments'
...
class StatsManager(object):
    ...
    def __init__(self, db_uri=None, db_name=DB_NAME,
    ...
    def setup_database_connection(self):
        ...
        self.experiments.ensure_index(self.marshalled_key, unique=True)

    def get_normalized_and_key(self, config):
        normalized = self.normalizer(deepcopy(config))
        serialized = json.dumps(normalized, sort_keys=True)
        return normalized, hashlib.md5(serialized).hexdigest()
    ...
    def book_if_available(self, experiment_configuration):
        ...
        try:
        ...
        except self.normalizer.UnHashableDict, e:
            logger.critical(
            ...
            if self.keep_running_on_errors:
                # Act like the experiment was already booked
                return None
            else:
                raise
        ...
        try:
            ticket = self.experiments.insert(normalized_config)
            ...
            experiment = self.experiments.find_and_modify(
                query, update=update,
    ...
    def store_results(self, booking_ticket, results):
        ...
        experiment = self.experiments.find_and_modify(query, update)
    ...
    def iter_results(self):
        return self.experiments.find({self.experiment_status: self.STATUS_SOLVED})
